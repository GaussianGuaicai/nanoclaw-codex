/**
 * Step: environment — Detect OS, local worker availability, existing config.
...
export async function run(_args: string[]): Promise<void> {
  ...
  const workerBuild: 'built' | 'source_only' | 'missing' = fs.existsSync(
    workerDist,
  )
    ? 'built'
    : fs.existsSync(workerSrc)
      ? 'source_only'
      : 'missing';
