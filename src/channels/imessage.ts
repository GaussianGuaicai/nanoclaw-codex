...
export function isValidIMessageTarget(value: string): boolean {
  ...
  const localValid = local.split('').every((ch) => /[A-Za-z0-9._+-]/.test(ch));
  const domainValid =
    domain.includes('.') &&
    domain.split('').every((ch) => /[A-Za-z0-9.-]/.test(ch));
...
class BlueBubblesBackend implements IMessageBackend {
  ...
  async sendText(_stableChatId: string, _text: string): Promise<void> {
    logger.warn(
      'Ignoring outbound iMessage send because bluebubbles is not implemented',
    );
  }
