...
describe('GroupQueue', () => {
  ...
  it('only runs one worker per group at a time', async () => {
  ...
  it('does NOT preempt active worker when not idle', async () => {
    ...
    // Enqueue a task while the worker is active but NOT idle
    const taskFn = vi.fn(async () => {});
    queue.enqueueTask('group1@g.us', 'task-1', taskFn);

    // _close should NOT have been written (worker is working, not idle)
  ...
  it('preempts idle worker when task is enqueued', async () => {
    const fs = await import('fs');
    let resolveProcess: () => void;
    ...
    // _close SHOULD have been written (worker is idle)
    const closeWrites = writeFileSync.mock.calls.filter(
      (call) => typeof call[0] === 'string' && call[0].endsWith('_close'),
    );
  ...
  it('sendMessage returns false for task workers so user messages queue up', async () => {
    let resolveTask: () => void;
    ...
    // Start a task (sets isTaskWorker = true)
    queue.enqueueTask('group1@g.us', 'task-1', taskFn);
    await vi.advanceTimersByTimeAsync(10);
    queue.registerProcess('group1@g.us', {} as any, 'worker-1', 'test-group');

    // sendMessage should return false — user messages must not go to task workers
  ...
  it('preempts when idle arrives with pending tasks', async () => {
    ...
    // Now the worker becomes idle — should preempt because a task is pending
    writeFileSync.mockClear();
    queue.notifyIdle('group1@g.us');
