...
export interface SummaryMemoryState {
  task: string;
  decisions: string[];
  constraints: string[];
...
export interface SummaryMemoryDocument {
  session_state: SummaryMemoryState;
}
...
export interface SummaryMemoryConfig {
  ...
  model: string;
  reasoningEffort: AgentModelReasoningEffort;
  updateMinTurns: number;
  maxItemsPerList: number;
}

export interface CompactionTriggerConfig {
  lastInputTokensGte: number;
  fallbackEstimatedTokensGte: number;
}

export interface CompactionWindowConfig {
  keepRecentTurns: number;
  keepRecentEstimatedTokens: number;
}
...
export interface ContextSourceConfig {
  chat: boolean;
  scheduledGroupContext: boolean;
  websocketGroupContext: boolean;
  isolatedTasks: boolean;
}

export interface ContextConfig {
  enabled: boolean;
  summaryMemory: SummaryMemoryConfig;
...
export type ContextTurnSource = AgentTaskSource;
...
export interface NewContextTurn {
  group_folder: string;
  chat_jid: string;
  source: ContextTurnSource;
  role: ContextTurnRole;
...
export interface GroupMemoryState {
  ...
  summary_yaml: string;
  last_summarized_turn_id: number;
  last_compacted_turn_id: number;
